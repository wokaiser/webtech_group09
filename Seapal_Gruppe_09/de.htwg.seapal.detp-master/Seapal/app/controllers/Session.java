...
public class Session extends Controller {
            ...
            try {
                ...
                while (result.next()) {
                    for (int i = 1; i < numColumns + 1; i++) {
                        ...
                        else if (columnName.equals("mySession")) {
                            cookieLessSession = result.getString(i);
                        }
                    }
                }
                ...
                if(pwCorrect) {
                    ...
                    session("bnr", "" + bnr);
                    respJSON.put("username", data.get("user_username"));
                    respJSON.put("mySession", cookieLessSession);
                    return ok(respJSON);
                }
           } catch (Exception e) { }
		}
	...
	public static Result logout() {
        ...
        if(conn != null) {
            ...
            try {
                query = conn.createStatement();
                sql = "UPDATE seapal.benutzer SET `mySession` = '" + data.get("mySession") + "' WHERE `bnr` = '" + session().get("bnr") + "'";
